"""lean_surface.py - Expected Lean public surface for se-theory-structural-explainability.
...
This module mirrors StructuralExplainability.lean so Python validation can check
that reference artifacts cover the public Lean surface.

Current strategy:
  Keep this file aligned manually with StructuralExplainability.lean.

Future strategy:
  Replace or supplement these constants by parsing StructuralExplainability.lean
  directly.
...
def expected_symbols_for_kind(kind: str) -> frozenset[str]:
    ...
    Args:
        kind: Surface kind. Expected values are type, predicate, witness,
            theorem.
